...
### Desempenho aprimorado de plot

--

### Objetos sf podem ser tratados como data frames (tibble) na maioria das operações
...
```{r,include=FALSE}
# directory
setwd("/home/mude/data/github/disciplina-geoprocessamento/03_dados/01_vetor/recife_raw/USO")

# import
rec_uso <- sf::st_read("PE_2611606_USO.shp", quiet = TRUE)
```
...
# import
rec_uso <- sf::st_read("PE_2611606_USO.shp")
```
...
# 8.7 Operações de atributos de vetores
...
## Todas as funções do dplyr para manejo de dados tabela de atributos
...
```{r, echo=FALSE, fig.align='center', out.width='50%'}
# plot
plot(rec_bairros_simp[1], plot = NA)
```
...
rec_bairros_grid_count <- rec_bairros_grid %>% 
  dplyr::mutate(n = sf::st_intersects(rec_bairros_grid, rec_bairros_rp) %>% lengths())
...
# import
per_bairros <- sf::st_read("Bairros.shp", quiet = TRUE)

# plot
plot(per_bairros[1], main = NA)
```
...
# bairros recife
per_bairros <- sf::st_read("Bairros.shp")
...
```{r}
# reprojection
per_bairros_geo <- sf::st_transform(per_bairros, 4326)
```
...
```{r}
# reprojection
sf::st_write(per_bairros_geo, "bairros_gcs_wgs84.shp")
```
